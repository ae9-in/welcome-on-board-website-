...
export default function ResetPasswordPage({ params }: { params: { token: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      // In production/dev this route updates the password using token
      // We will create the API endpoint right inside or mock it.
      ...
      if (!res.ok) {
        setError(data.error || 'Reset failed or token has expired.');
        return;
      }
      setSuccess(true);
      setTimeout(() => {
      ...
      >
        <div className="bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
          <div className="text-center mb-8">
            ...
            <p className="text-white/40 text-sm mt-1">Enter your new account password</p>
          </div>
