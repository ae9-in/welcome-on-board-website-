...
export default function Competitions({ navigateTo, setSelectedComp, activeFigurineBg }) {
  ...
  const isRegistered = (compTitle) => {
    return true; // Bypass lock to allow direct entry
  };
  ...
  const handleStart = (comp) => {
    ...
    const studentName = reg?.studentName || localStorage.getItem('onboreding_student_name') || 'Guest Speller';
    const grade = reg?.grade || '5th Standard';
    
    // Pass subject so Quiz knows to filter to math-only or general
    const subject = comp.id === 'math' ? 'math' : 'general';
    navigateTo(comp.route, { grade, comp: comp.title, name: studentName, subject });
  };
